    ...
    <div className="space-y-6">
      ...
      {locations.length === 0 ? (
        <EmptyState
          ...
          description="Save cities from the Weather page to monitor local risk cards and nearby earthquakes."
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {locations.map((l) => (
            <div key={l.id} className="rounded-lg border p-4">
              ...
              <Button variant="outline" size="sm" className="mt-3" onClick={() => remove(l.id)}>
                Remove
              </Button>
            </div>
          ))}
